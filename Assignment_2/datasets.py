...
class dataset(Dataset):
    ...
    def __getitem__(self, idx): # 1.25 Mark
        data_dict = {}
        # statement = word_tokenize(self.dataframe.loc[[idx][3]].lower())
        # justification = word_tokenize(self.dataframe.loc[[idx][15]].lower())
        # label = torch.tensor(self.labels[self.dataframe.loc[[idx][2]]])
        ...
        credit_history = torch.tensor(numpy.asarray([self.dataframe.iloc[idx][12],self.dataframe.iloc[idx][11],self.dataframe.iloc[idx][9],self.dataframe.iloc[idx][10],self.dataframe.iloc[idx][13]]))

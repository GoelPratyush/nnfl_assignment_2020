...
class arePantsonFire(nn.Module):
    ...
    def forward(self, sentence, justification, credit_history): # 1 Marks
        ...
        encoded_explanation = self.explanation_encoder.forward(justification)
        attention_output = self.attention.forward(encoded_sentence, encoded_explanation, encoded_explanation)
        flat = self.flatten_conv(nn.functional.relu(self.first_conv(self.position_feedforward.forward(attention_output)), inplace=False))
        flat_layer = nn.Flatten()
        flat = flat_layer(flat)
        return self.classifier(self.bilinear(self.linear2(self.linear1(flat)), credit_history))
